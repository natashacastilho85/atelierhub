...
// Retorna false se já foi disparada antes (UNIQUE constraint)
async function disparar(userId, chave, titulo, corpo) {
  ...
  await enviarPush(userId, titulo, corpo);
...
async function verificarNotificacoes() {
  ...
  for (const c of contratos) {
    ...
    if (addDays(cas, -7) === today)
      await disparar(uid, `7d-${id}`, '💍 Casamento em 7 dias', `${c.nome} — o grande dia está chegando!`);
    ...
    if (addDays(cas, -20) === today && c.forma_pag === 'reserva') {
      ...
      if (saldo > 0.01)
        await disparar(uid, `reserva20-${id}`, '💰 Lembrete reserva 20 dias',
          `${c.nome} — saldo de R$${saldo.toFixed(2).replace('.', ',')} em aberto`);
    }

    // 3. 📦 Confirmação de envio — 1 dia útil após casamento
    if (addBusinessDays(cas, 1) === today)
      await disparar(uid, `envio1du-${id}`, '📦 Confirmação de envio', `${c.nome} — confirme o envio do buquê`);

    // 4, 5, 6. 🌸 Buquê chegou? — só se não tiver data de chegada
    if (!c.chegada) {
      if (addBusinessDays(cas, 3) === today)
        await disparar(uid, `buque3du-${id}`, '🌸 Buquê chegou?', `${c.nome} — 3 dias úteis após o casamento`);
      if (addBusinessDays(cas, 5) === today)
        await disparar(uid, `buque5du-${id}`, '🌸 Buquê chegou?', `${c.nome} — 5 dias úteis após o casamento`);
      if (addBusinessDays(cas, 8) === today)
        await disparar(uid, `buque8du-${id}`, '🌸 Buquê chegou?', `${c.nome} — 8 dias úteis, sem confirmação de chegada`);
    }
    ...
    if (c.entrega && c.chegada) {
      ...
      if (addDays(entrega, -20) === today)
        await disparar(uid, `prazo20-${id}`, '⏰ Prazo de produção', `${c.nome} — faltam 20 dias para a entrega`);
    }

    // 8. 💍 Dia do casamento
    if (cas === today)
      await disparar(uid, `casamento-${id}`, '💍 Dia do casamento!', `Hoje é o grande dia de ${c.nome}! 🌸`);
  }
